    ...
    return (
        <div className="p-6 max-w-6xl mx-auto space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                ...
                <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
                    <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Resolución Promedio</p>
                ...
                <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
                    <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Costo Promedio (30d)</p>
            ...
            {mostFrequentCategory && (
                <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
                    ...
                    <p className="text-xl font-semibold text-gray-900 dark:text-white capitalize">
                        {CATEGORIES[mostFrequentCategory as keyof typeof CATEGORIES]?.label || mostFrequentCategory.replace('_', ' ')} ({categoryCount[mostFrequentCategory]} tickets)
                    </p>
                </div>
            )}
            ...
            {blocks && blocks.length > 0 && (
                <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-6">
                    <h3 className="text-lg font-semibold text-red-900 dark:text-red-200 mb-4">
                        🚫 Unidades Bloqueadas ({blocks.length})
                    </h3>
                                    ...
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <p className="font-medium text-gray-900 dark:text-white">
                                                {block.unit?.name || 'Toda la Propiedad'}
                                            </p>
                                            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                                                {block.reason}
                                            </p>
                                            <p className="text-xs text-gray-500 dark:text-gray-500 mt-1">
                                                Bloqueado por {block.blocked_by_profile?.full_name}
                                            </p>
                                        </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow">
                <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                    <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
                        Unidades ({property.units?.length || 0})
                    </h2>
                </div>
                <div className="p-6">
                    {property.units && property.units.length > 0 ? (
                                ...
                                >
                                    <div className="flex justify-between items-start">
                                        ...
                                        {urgentUnitIds.has(unit.id) && (
                                            <div title="Tarea Urgente Activa">
                                                <UrgentIndicator />
                                            </div>
                                        )}
                                    </div>
                        ...
                        <p className="text-gray-500 dark:text-gray-400">No hay unidades definidas</p>
                    )}
                </div>
            </div>
        </div>
